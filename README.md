...
Sharkdown 是一个纯静态、离线优先的 **Share Markdown 工作台**。用户在浏览器里编辑 Markdown，实时预览排版效果，并把内容交付为图片、文字版 PDF、自包含 HTML、纯 Markdown、轻量分享链接或 `.sharkdown` 可继续编辑工程包。用户内容不上传，所有渲染、检查、打包和导出都在本地浏览器完成。
...
-> image/pdf/html/project/url artifact -> download / clipboard / native share
```
