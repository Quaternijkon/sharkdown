...
- 三栏工作台：左侧 Markdown 编辑，中间渲染预览，右侧为 VS Code 风格活动栏，可在转换导出、外观样式、分析评估和文件系统之间切换。
- 可调工作区：桌面端可拖拽编辑器/预览、预览/右侧栏之间的分隔条，并可折叠右侧栏保留活动图标。
...
- 独立分析评估页：识别语法使用情况、代码块语言、章节占比、任务完成度、基础规模、阅读时长、语法覆盖率、标题层级、内容构成，并给出结构清晰度、内容完整度、可视化潜力、分享可用性和阅读友好度等本地启发式评分。
